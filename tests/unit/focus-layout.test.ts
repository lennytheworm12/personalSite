...
import {
  FOCUS_LAYOUTS,
  resolveFocusLayout,
  validateFocusLayouts,
} from "@/graph/focus-layouts";
...
describe("project-focus layouts", () => {
  ...
  it("rejects an unknown coordinate ID", () => {
    ...
    expect(() =>
      validateFocusLayouts(homeGraph, broken, {
        featuredSlugs: FEATURED_PROJECT_SLUGS,
      }),
    ).toThrow(
      /unknown coordinate ID in focus layout wide\/spotify-sorter: project:ghost/,
    );
  });
  ...
  it("rejects out-of-bounds coordinates", () => {
    ...
    expect(() =>
      ...
      /out-of-bounds coordinate for tech:typescript in focus layout wide\/game-teacher/,
    );
  });
  ...
  it("keeps related story and technology nodes near the focus", () => {
    const broken = structuredClone(FOCUS_LAYOUTS);
    broken.laptop["game-teacher"]!.nodes["story:game-teacher:motivation"] = {
      x: 2,
      y: 98,
    };
    expect(() =>
      ...
      /related node too far from focus in laptop\/game-teacher: story:game-teacher:motivation/,
    );
  });
